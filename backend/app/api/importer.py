...
from app.services.importer import import_recipe_url
...
async def import_url(payload: UrlImportRequest, db: Session = Depends(get_db)):
    ...
    parsed = [
        ImportedIngredientDraft(**parse_ingredient_line(line).__dict__)
        for line in raw.get("raw_ingredients", [])
    ]

    duplicates = find_possible_duplicates(
        db,
        raw.get("name") or "Untitled Recipe",
        [item.name for item in parsed],
    )

    return {
        **raw,
        "ingredients": [item.model_dump() for item in parsed],
...
def save_imported_recipe(
    payload: SaveImportedRecipeRequest,
    ...
    recipe = Recipe(
        ...
        instructions="\n".join(step.strip() for step in payload.instructions if step.strip()),
    ...
    for order, item in enumerate(payload.ingredients, start=1):
        ...
        unit = None
        if item.unit:
            unit = db.scalar(select(Unit).where(Unit.abbreviation == item.unit))
