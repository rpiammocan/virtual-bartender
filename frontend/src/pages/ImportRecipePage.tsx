...
export default function ImportRecipePage({onHome,openRecipe}:Props){
  ...
  function updateManualIngredient(index:number,key:keyof ManualIngredient,value:string|number|null){setManual(current=>{const ingredients=[...current.ingredients];ingredients[index]={...ingredients[index],[key]:value};return{...current,ingredients}})}
  function addManualIngredient(){setManual(current=>({...current,ingredients:[...current.ingredients,{quantity:1,unit:"oz",name:""}]}))}
  function removeManualIngredient(index:number){setManual(current=>({...current,ingredients:current.ingredients.filter((_,i)=>i!==index)}))}
  ...
  async function saveManual(){
    ...
    if(!manual.name.trim()||ingredients.length===0){setError("Enter a recipe name and at least one ingredient.");return;}
    ...
    }catch(err){setError(err instanceof Error?err.message:"Unable to save recipe.")}finally{setBusy(false)}
  }
  ...
  return <main className="page theme-recipes">
    ...
    <div className="toolbar">
      <button className={mode==="manual"?"primary":""} onClick={()=>setMode("manual")}>Manual Add</button>
    ...
    <p className="lede">{mode==="manual"?"Type in your own recipe and save it directly to Virtual Bartender.":mode==="single"?"Paste one recipe URL. Review and edit everything before saving it locally.":"Paste a recipe collection page. Scan it, select recipes, and save them to your offline Virtual Bartender database."}</p>

    {mode!=="manual"&&<div className="toolbar"><input className="wide-input" placeholder="https://..." value={url} onChange={e=>setUrl(e.target.value)}/><button className="primary" disabled={busy||!url.trim()} onClick={()=>mode==="single"?importRecipe():scan()}>{busy?"Working…":mode==="single"?"Import & Review":"Scan Website"}</button></div>}
    {error&&<p className="error">{error}</p>}

    {mode==="manual"&&<section className="detail-card"><p className="eyebrow">Manual recipe entry</p><label>Recipe name<input className="wide-input" value={manual.name} onChange={e=>setManual({...manual,name:e.target.value})} placeholder="Recipe name"/></label><label> Type <select value={manual.recipe_type} onChange={e=>setManual({...manual,recipe_type:e.target.value as "cocktail"|"mocktail"})}><option value="cocktail">Cocktail</option><option value="mocktail">Mocktail</option></select></label><h3>Ingredients</h3><div className="import-ingredients">{manual.ingredients.map((item,index)=><div className="import-row" key={index}><input value={item.quantity??""} onChange={e=>updateManualIngredient(index,"quantity",e.target.value===""?null:Number(e.target.value))} placeholder="Qty"/><select value={item.unit??""} onChange={e=>updateManualIngredient(index,"unit",e.target.value||null)}><option value="">Unit</option><option value="oz">oz</option><option value="ml">ml</option><option value="tsp">tsp</option><option value="tbsp">tbsp</option><option value="dash">dash</option><option value="pc">pc</option><option value="cup">cup</option></select><input value={item.name} onChange={e=>updateManualIngredient(index,"name",e.target.value)} placeholder="Ingredient"/><button className="danger-link" type="button" onClick={()=>removeManualIngredient(index)}>Remove</button></div>)}</div><div className="toolbar"><button type="button" onClick={addManualIngredient}>+ Ingredient</button></div><h3>Instructions</h3><textarea rows={8} value={manual.instructions.join("\n")} onChange={e=>setManual({...manual,instructions:e.target.value.split("\n").filter(x=>x.trim())})} placeholder="Enter each step on a new line"/><div className="toolbar"><button className="primary" disabled={busy||!manual.name.trim()} onClick={saveManual}>{busy?"Saving…":"Save Recipe"}</button></div>{savedId!==null&&<p className="success">Recipe saved successfully. {openRecipe&&<button className="link-button" onClick={()=>openRecipe(savedId)}>View Recipe</button>}</p>}</section>}
