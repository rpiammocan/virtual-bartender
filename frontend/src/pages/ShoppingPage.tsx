          ...
          {suggestions.length === 0 ? (
            ...
            <article className="recipe-card" key={item.ingredient_name}>
              ...
              <button onClick={async () => {
                await api.shopping.addManual(item.ingredient_name, item.category);
                await load();
              }}>+ Add</button>
            </article>
          ))}
          ...
          <div className="inventory-list">
            {categoryItems.map((item) => (
              <article className="inventory-row" key={item.id}>
                ...
                <button className="danger-link" onClick={async () => { await api.shopping.remove(item.id); await load(); }}>Remove</button>
              </article>
            ))}
          </div>
        </section>
